...
use_tf12_api = version.parse(tf.VERSION) >= version.parse('0.12.0')
...
def env_sampler(env, policy, num_local_steps):
    """
The logic of the thread runner.  In brief, it constantly keeps on running
the policy, and as long as the rollout exceeds a certain length, the thread
runner appends the policy to the queue.
"""
    ...
    while not (terminal_end or terminal):
        ...
        # collect the experience
        rollout.add(last_state, action.reshape((-1,1)), reward, value_, terminal, last_features)
        length += 1
        rewards += reward
